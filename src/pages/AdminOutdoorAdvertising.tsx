...
interface SortableItemUIProps extends React.HTMLAttributes<HTMLLIElement> {
    item: OutdoorAdvertisingItem;
    onEdit: (item: OutdoorAdvertisingItem) => void;
...
const EditModal = ({ item, isOpen, onClose, onSave, loading }: { item: OutdoorAdvertisingItem | null; isOpen: boolean; onClose: () => void; onSave: (item: OutdoorAdvertisingItem) => void; loading: boolean }) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-40 animate-fade-in">
            <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-slate-800">{formData.id ? 'Edit' : 'Add'} Outdoor Ad</h3>
...
const SortableItem = ({ item, onEdit, onRemove }: { item: OutdoorAdvertisingItem, onEdit: (item: OutdoorAdvertisingItem) => void, onRemove: (id: string) => void}) => {
...
const AdminOutdoorAdvertising = () => {
    const [outdoorItems, setOutdoorItems] = useState<OutdoorAdvertisingItem[]>([]);
    const [loading, setLoading] = useState(true);
    const [isDirty, setIsDirty] = useState(false);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingItem, setEditingItem] = useState<OutdoorAdvertisingItem | null>(null);
    ...
    const fetchItems = useCallback(async () => {
        ...
        const items = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as OutdoorAdvertisingItem));
        items.sort((a, b) => a.order - b.order);
        setOutdoorItems(items);
        setLoading(false);
        setIsDirty(false);
    }, []);
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            setOutdoorItems((items) => arrayMove(items, items.findIndex(i => i.id === active.id), items.findIndex(i => i.id === over.id)));
            setIsDirty(true);
        }
    };
    ...
    const handleSaveOrder = async () => {
        ...
        outdoorItems.forEach((item, index) => {
            batch.update(doc(db, 'outdoorAdvertising', item.id), { order: index });
        });
        try {
            await batch.commit();
    ...
    const handleSaveItem = async (itemToSave: OutdoorAdvertisingItem) => {
        setLoading(true);
        try {
            if (itemToSave.id) {
                await updateDoc(doc(db, 'outdoorAdvertising', itemToSave.id), { ...itemToSave });
                showNotification('Item updated successfully!', 'success');
            } else {
                await addDoc(collection(db, 'outdoorAdvertising'), itemToSave);
    ...
    const handleRemoveItem = async (id: string) => {
        if (window.confirm('Are you sure you want to delete this item?')) {
            try {
                await deleteDoc(doc(db, 'outdoorAdvertising', id));
    ...
    return (
        <div className="animate-fade-in">
            ...
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-3xl font-bold text-slate-800">Manage Outdoor Ads</h2>
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => handleOpenModal()} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow-sm">
                        <PlusCircle size={18} /> Add Ad
                    </button>
                </div>
            </div>

            {loading && outdoorItems.length === 0 ? (
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                    <SortableContext items={outdoorItems.map(m => m.id)} strategy={verticalListSortingStrategy}>
                        <ul className="space-y-3">
                            {outdoorItems.map(item => <SortableItem key={item.id} item={item} onRemove={handleRemoveItem} onEdit={handleOpenModal} />)}
                        </ul>
                    </SortableContext>
                </DndContext>
            ) : (
                <div className="text-center py-16 bg-white rounded-xl shadow-md border border-slate-200">
                    ...
                    <h3 className="mt-2 text-xl font-semibold text-slate-700">No Outdoor Ads Found</h3>
                    <p className="mt-1 text-sm text-slate-500">Click "Add Ad" to get started.</p>
                </div>
            )}
        </div>
    );
...
export default AdminOutdoorAdvertising;