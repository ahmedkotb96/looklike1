...
import { sanitizeInput, validateImageUrl } from '@/utils/security';
...
const EditModal = ({ item, isOpen, onClose, onSave, loading }: { item: PortfolioItem | null; isOpen: boolean; onClose: () => void; onSave: (item: PortfolioItem) => void; loading: boolean }) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-40 animate-fade-in">
            <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-slate-800">{formData.id ? 'Edit' : 'Add'} Portfolio Item</h3>
                    <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-100"><X size={20} /></button>
                </div>
                <div className="space-y-4">
                    <input type="text" name="title" value={formData.title} onChange={handleChange} placeholder="Title" className="w-full px-4 py-2 border border-slate-300 rounded-lg" />
    ...
    ({ item, onEdit, onRemove, listeners, ...props }, ref) => (
        <li ref={ref} {...props} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm border border-slate-200">
            <div className="flex items-center gap-4">
                ...
                <img src={item.image} alt={item.title} className="w-16 h-12 rounded-md object-cover bg-slate-100" />
                <div>
...
const SortableItem = ({ item, onEdit, onRemove }: { item: PortfolioItem, onEdit: (item: PortfolioItem) => void, onRemove: (id: string) => void}) => {
...
const AdminPortfolio = () => {
    ...
    useEffect(() => { fetchPortfolioItems(); }, [fetchPortfolioItems]);

    const handleDragEnd = (event: DragEndEvent) => {
        const { active, over } = event;
        if (over && active.id !== over.id) {
            setPortfolioItems((items) => arrayMove(items, items.findIndex(i => i.id === active.id), items.findIndex(i => i.id === over.id)));
            setIsDirty(true);
        }
    };
    ...
    const handleSaveItem = async (itemToSave: PortfolioItem) => {
        setLoading(true);
        try {
            // Validate image URLs before saving
            if (!validateImageUrl(itemToSave.image)) {
                showNotification('Invalid image URL. Please use a valid image from a trusted source.', 'error');
                setLoading(false);
                return;
            }
            if (itemToSave.logo && !validateImageUrl(itemToSave.logo)) {
                showNotification('Invalid logo URL. Please use a valid image from a trusted source.', 'error');
                setLoading(false);
                return;
            }
        ...
        <div className="animate-fade-in">
            ...
            {loading && portfolioItems.length === 0 ? (
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                    <SortableContext items={portfolioItems.map(m => m.id)} strategy={verticalListSortingStrategy}>
                        <ul className="space-y-3">
                            {portfolioItems.map(item => <SortableItem key={item.id} item={item} onRemove={handleRemoveItem} onEdit={handleOpenModal} />)}
                        </ul>
                    </SortableContext>
                </DndContext>
            ) : (
                <div className="text-center py-16 bg-white rounded-xl shadow-md border border-slate-200">
                    ...
                    <h3 className="mt-2 text-xl font-semibold text-slate-700">No Portfolio Items Found</h3>
