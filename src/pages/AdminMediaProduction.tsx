...
interface SortableItemUIProps extends React.HTMLAttributes<HTMLLIElement> {
    item: MediaProductionItem;
    onEdit: (item: MediaProductionItem) => void;
...
const EditModal = ({ item, isOpen, onClose, onSave, loading }: { item: MediaProductionItem | null; isOpen: boolean; onClose: () => void; onSave: (item: MediaProductionItem) => void; loading: boolean }) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-40 animate-fade-in">
            <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-slate-800">{formData.id ? 'Edit' : 'Add'} Media Item</h3>
                ...
                <div className="space-y-4">
                    ...
                    <input type="text" name="mediaUrl" value={formData.mediaUrl} onChange={handleChange} placeholder="Media URL (Image or Video)" className="w-full px-4 py-2 border border-slate-300 rounded-lg" />
                </div>
...
const SortableItemUI = React.forwardRef<HTMLLIElement, SortableItemUIProps>(
    ({ item, onEdit, onRemove, listeners, ...props }, ref) => {
        const isVideo = item.mediaUrl.match(/\.(mp4|webm|ogg)$/i);
        return (
            <li ref={ref} {...props} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm border border-slate-200">
                <div className="flex items-center gap-4">
                    ...
                    {isVideo ? (
                        <video src={item.mediaUrl} className="w-16 h-12 rounded-md object-cover bg-slate-900" muted loop />
                    ) : (
...
const SortableItem = ({ item, onEdit, onRemove }: { item: MediaProductionItem, onEdit: (item: MediaProductionItem) => void, onRemove: (id: string) => void}) => {
...
const AdminMediaProduction = () => {
    const [mediaItems, setMediaItems] = useState<MediaProductionItem[]>([]);
    const [loading, setLoading] = useState(true);
    const [isDirty, setIsDirty] = useState(false);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingItem, setEditingItem] = useState<MediaProductionItem | null>(null);
    ...
    const fetchItems = useCallback(async () => {
        ...
        const items = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as MediaProductionItem));
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            setMediaItems((items) => arrayMove(items, items.findIndex(i => i.id === active.id), items.findIndex(i => i.id === over.id)));
            setIsDirty(true);
        }
    };
    ...
    const handleSaveOrder = async () => {
        ...
        mediaItems.forEach((item, index) => {
            batch.update(doc(db, 'mediaProduction', item.id), { order: index });
        });
        try {
            await batch.commit();
    ...
    const handleOpenModal = (item: MediaProductionItem | null = null) => {
        setEditingItem(item || { id: '', name: '', description: '', mediaUrl: '', order: mediaItems.length });
        setIsModalOpen(true);
    };

    const handleSaveItem = async (itemToSave: MediaProductionItem) => {
        setLoading(true);
        try {
            if (itemToSave.id) {
                await updateDoc(doc(db, 'mediaProduction', itemToSave.id), { ...itemToSave });
                showNotification('Item updated successfully!', 'success');
            } else {
                await addDoc(collection(db, 'mediaProduction'), itemToSave);
    ...
    const handleRemoveItem = async (id: string) => {
        if (window.confirm('Are you sure you want to delete this item?')) {
            try {
                await deleteDoc(doc(db, 'mediaProduction', id));
    ...
    return (
        <div className="animate-fade-in">
            ...
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-3xl font-bold text-slate-800">Manage Media Production</h2>
            ...
            {loading && mediaItems.length === 0 ? (
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                    <SortableContext items={mediaItems.map(m => m.id)} strategy={verticalListSortingStrategy}>
                        <ul className="space-y-3">
                            {mediaItems.map(item => <SortableItem key={item.id} item={item} onRemove={handleRemoveItem} onEdit={handleOpenModal} />)}
                        </ul>
                    </SortableContext>
                </DndContext>
            ) : (
                <div className="text-center py-16 bg-white rounded-xl shadow-md border border-slate-200">
                    ...
                    <h3 className="mt-2 text-xl font-semibold text-slate-700">No Media Items Found</h3>
...
export default AdminMediaProduction;